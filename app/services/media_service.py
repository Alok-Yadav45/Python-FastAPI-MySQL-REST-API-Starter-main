...
from app.schemas.media_schema import MediaCreate, Media
...
MAX_FILE_SIZE_MB = {"image": 15, "video": 50, "document": 10, "other": 20}
...
def upload_media(db: Session, files: list[UploadFile], entity_type: str, entity_id: int, file_type: str):
    ...
    for file in files:
        ...
        uploaded.append(Media.from_orm(media))

    return success_response(data=uploaded, message=f"{len(uploaded)} {file_type}(s) uploaded successfully")
